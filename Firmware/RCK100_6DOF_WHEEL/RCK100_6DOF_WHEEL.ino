...
#define SERVO_7_WHEEL_ID 7
#define SERVO_8_WHEEL_ID 8
...
#define SERVO_5_ROW_MAX 896
#define SERVO_5_ROW_MIN 128
...
#define WHEEL_SPEED_L 512
#define WHEEL_SPEED_H 1023
#define POS_DELTA 20
...
static int _servo_6_grip_pos = SERVO_6_GRIPPER_MIN;
static boolean _hand_status = false;
static boolean ID_5_row = false; 
   ...
   if(Serial2.available() > 0){
    ...
    if(BT_update){
      ...
      if(packet[1]!=255 && packet[2]!=1){
        ...
        //L1 Button
            ...
            if(RCU_L1 != 0){Action(RCU_L1);}
              ...
              _servo_3_pitch_delta = POS_DELTA_RCU;
            }
        }
        ...
        //L3 Button
          ...
          if(RCU_L3 != 0){Action(RCU_L3);}
            ...
            if(ID_5_row) SetPositionI_JOG(SERVO_5_ROW_ID, SETTING_TIME_GRIPPER, 790);
  	    else SetPositionI_JOG(SERVO_5_ROW_ID, SETTING_TIME_GRIPPER, 512);  
            ID_5_row = !ID_5_row;
          }
        }
        ...
        //R1 Button
            ...
            if(RCU_R1 != 0){Action(RCU_R1);}
              ...
              _servo_4_pitch_delta = POS_DELTA_RCU;
            }
        }
        ...
        //R3 Button
          ...
          if(RCU_R3 != 0){Action(RCU_R3);}
            ...
            if(_hand_status) {
              for(int f = 1;f < 11;f++){
                 ReadDataRAM2(SERVO_6_GRIPPER_ID, RAM_Bus_Current) ; 
                 if(ReadDataRAM2(SERVO_6_GRIPPER_ID, RAM_Bus_Current)>200) {SetPositionI_JOG(SERVO_6_GRIPPER_ID, SETTING_TIME_GRIPPER, SERVO_6_GRIPPER_MAX-((f-2)*34));break;}
                 else SetPositionI_JOG(SERVO_6_GRIPPER_ID, SETTING_TIME_GRIPPER, SERVO_6_GRIPPER_MAX-(f*34));
                 delay(100);
            }
          }
  	     else SetPositionI_JOG(SERVO_6_GRIPPER_ID, SETTING_TIME_GRIPPER, SERVO_6_GRIPPER_MAX);
  	     _hand_status = !_hand_status;
          }
        } 
        //LightJoystick_Rightside
      ...
      //==== App Command ==== 
        ...
        if(packet[3] == 102) A1_16_TorqueOff(254);       
        ...
        else if(packet[3] == 16) {//mode3 direction
          _servo_1_yaw_pos -= POS_DELTA;
          if(_servo_1_yaw_pos <= SERVO_1_YAW_MIN) {_servo_1_yaw_pos = SERVO_1_YAW_MIN;}
          SetPositionI_JOG(SERVO_1_YAW_ID, SETTING_TIME, _servo_1_yaw_pos); 
        }
        else if(packet[3] == 39) { //APP mode 3 page1 button A
        ...
        else if(packet[3] == 40) { //APP mode 3 page1 button D
          _servo_3_pitch_pos += POS_DELTA;
          if(_servo_3_pitch_pos >= SERVO_3_PITCH_MAX) {_servo_3_pitch_pos = SERVO_3_PITCH_MAX;}
          SetPositionI_JOG(SERVO_3_PITCH_ID, SETTING_TIME, _servo_3_pitch_pos); 
        }
        else if(packet[3] == 38) { //APP mode 3 page1 button B
         ...
         else if(packet[3] == 46) { //APP mode 3 page1 button E
          _servo_4_pitch_pos += POS_DELTA;
          if(_servo_4_pitch_pos >= SERVO_4_PITCH_MAX) {_servo_4_pitch_pos = SERVO_4_PITCH_MAX;}
          SetPositionI_JOG(SERVO_4_PITCH_ID, SETTING_TIME, _servo_4_pitch_pos); 
        }
        else if(packet[3] == 41) { //APP mode 3 page1 button C
         ...
         else if(packet[3] == 47) { //APP mode 3 page1 button F
           _servo_6_grip_pos += POS_DELTA;
          if(_servo_6_grip_pos >= SERVO_6_GRIPPER_MAX) {_servo_6_grip_pos = SERVO_6_GRIPPER_MAX;}
          SetPositionI_JOG(SERVO_6_GRIPPER_ID, SETTING_TIME_GRIPPER, _servo_6_grip_pos); 
        }
        else if(packet[3] == 1) { //APP modw3 page1 UP
           wheel_active(3,WHEEL_SPEED_H);
        }
         else if(packet[3] == 2) { //APP modw3 page1 DOWN
           wheel_active(7,WHEEL_SPEED_H);
        }
        else if(packet[3] == 3) { //APP modw3 page1 LEFT
           wheel_active(5,WHEEL_SPEED_H);
        }
        else if(packet[3] == 4) { //APP modw3 page1 RIGHT
           wheel_active(1,WHEEL_SPEED_H);
        }
        else if(packet[3] == 7)  Action(50); //APP mode1 page1 A
        else if(packet[3] == 8)  Action(49); //APP mode1 page1 B
        else if(packet[3] == 21)  Action(48);//APP mode1 page1 C
        else if(packet[3] == 9)  Action(46);//APP mode1 page1 D
        else if(packet[3] == 10)  Action(47);//APP mode1 page1 E
        else if(packet[3] == 20)  Action(45);//APP mode1 page1 F
        else if(packet[3] == 22)  Action(44);//APP mode1 page2 A
        else if(packet[3] == 27)  Action(43);//APP mode1 page2 B
        else if(packet[3] == 28)  Action(42);//APP mode1 page2 C
        else if(packet[3] == 17)  Action(46);//APP mode1 page2 D
        else if(packet[3] == 18)  Action(47);//APP mode1 page2 E
        else if(packet[3] == 19)  Action(45);//APP mode1 page2 F
  	...
  	if(Timer_Task(LOOP_DELTA_TIME)){
		...
		static uint8_t _servo_3_start = 1;
                ...
                if(packet[1] > 0 && packet[2] > 0 && packet[1] != 255 && packet[2]!= 1 && (RCU_LJR == 0 && RCU_LJL == 0 && RCU_LJU == 0 && RCU_LJD == 0))   phase(packet[1] , packet[2]);
...
void AIM_Task_Setup(){
  ...
  const unsigned char SN[14] = {0x01, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00, 0x1B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
  for(int i = 0; i < 14; i++) { EEPROM.write(i+10, SN[i]); delay(5); }
}
...
void BUTTON_Task(void){
  ...
  if(button_timer > 20){
    ...
    if(key != 0){
      XYZrobot.readPose();
      if(last_key == key_mask_button1){// button 1
            if(RB_1 != 0)  { Action(RB_1);}
            else {Action(52);}
      }
      else if(last_key == key_mask_button2) {// button 2
            if(RB_2 != 0)  {Action(RB_2);}
            else {Action(53);}
      }
      else if(last_key == key_mask_button3) {// button 3
            if(RB_3 != 0)  {Action(RB_3);}
            else {Action(54);}
      }
      else if(last_key == key_mask_button4) {// button 4
            if(RB_4 != 0)  {Action(RB_4);}
            else {Action(51);wheel(1);}
      }
    }
  }
}
...
void wheel_active(int Z,int W){
    if(Z == 3){
         //Serial.println("F");
         A1_16_SetSpeed(7, 30, -W);//F　前進
         A1_16_SetSpeed(8, 30, W);
    }
    if(Z == 2){
         //Serial.println("F");
         A1_16_SetSpeed(7, 30, -(W/2));//F　　右前進
         A1_16_SetSpeed(8, 30, W);
    }
    if(Z == 4){
         //Serial.println("F");
         A1_16_SetSpeed(7, 30, -W);//F　　左前進
         A1_16_SetSpeed(8, 30, (W/2));
    }
    else if(Z == 7){
        //Serial.println("B");
        A1_16_SetSpeed(7, 30, W);//B　後退
        A1_16_SetSpeed(8, 30, -W);
    }
    else if(Z == 6){
        //Serial.println("R");
         A1_16_SetSpeed(7, 30, W);//R　　後左轉
         A1_16_SetSpeed(8, 30, -(W/2));
    }
    else if(Z == 8){
        //Serial.println("R");
         A1_16_SetSpeed(7, 30, (W/2));//R　　後右轉
         A1_16_SetSpeed(8, 30, -W);
    }
    else if(Z == 1){
        //Serial.println("R");
         A1_16_SetSpeed(7, 30, W);//R　右轉
         A1_16_SetSpeed(8, 30, W);
    }
    else if(Z == 5){
       // Serial.println("L");
         A1_16_SetSpeed(7, 30, -W);//L　左轉
         A1_16_SetSpeed(8, 30, -W);
    }
    else if(Z == 9){
        A1_16_SetSpeed(7, 30, 0);//stop　停止
        A1_16_SetSpeed(8, 30, 0);
    }
}
void wheel(int X){
    if(X == 1){
          A1_16_SetSpeed(7, 0, -WHEEL_SPEED_L);//F　前進
          A1_16_SetSpeed(8, 0, WHEEL_SPEED_L);
          delay(2000);
          A1_16_SetSpeed(7, 0, WHEEL_SPEED_L);//B　後退
          A1_16_SetSpeed(8, 0, -WHEEL_SPEED_L);
          delay(3000);
          wheel_active(9,0);
    }
}
void phase(int x,int y){
   if(x > 150 &&  y > 95 && y < 150) {quadrant = 1;wheel_active(1,WHEEL_SPEED_H);}
   else if(x > 150 && y >150) {quadrant = 2;;wheel_active(2,WHEEL_SPEED_H);}
   else if(x > 95 && x <150 && y > 150) {quadrant = 3;wheel_active(3,WHEEL_SPEED_H);}
   else if (x <= 95 && y > 150) {quadrant = 4;;wheel_active(4,WHEEL_SPEED_H);}
   else if(x < 95 && y > 95 && y < 150 ) {quadrant = 5;wheel_active(5,WHEEL_SPEED_H);}
   else if (x <= 95 && y <=95) {quadrant = 6;;wheel_active(6,WHEEL_SPEED_H);}
   else if(x > 95 && x <150 && y <95) {quadrant = 7;wheel_active(7,WHEEL_SPEED_H);}
   else if (x > 150 && y <= 95) {quadrant = 8;;wheel_active(8,WHEEL_SPEED_H);}
   else {quadrant = 9;wheel_active(9,0);}
}
void start_pos_setup(){
  SetPositionI_JOG(SERVO_1_YAW_ID, SETTING_TIME_HOME, _servo_1_yaw_pos);
  SetPositionI_JOG(SERVO_2_PITCH_ID, SETTING_TIME_HOME, _servo_2_pitch_pos);
  SetPositionI_JOG(SERVO_3_PITCH_ID, SETTING_TIME_HOME, _servo_3_pitch_pos);
  SetPositionI_JOG(SERVO_4_PITCH_ID, SETTING_TIME_HOME, _servo_4_pitch_pos);
  SetPositionI_JOG(SERVO_5_ROW_ID, SETTING_TIME_HOME, _servo_5_row_pos);
  SetPositionI_JOG(SERVO_6_GRIPPER_ID, SETTING_TIME_HOME, _servo_6_grip_pos);
}
